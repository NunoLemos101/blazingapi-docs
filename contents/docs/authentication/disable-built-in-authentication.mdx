...
Authentication views are responsible for handling user authentication in your application. To deactivate authentication, remove the `blazingapi.auth.views` module from the `VIEW_FILES` list in `settings.py`.

This will prevent the application from rendering the authentication endpoints.
